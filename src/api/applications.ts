  ...
  try {
    if (!remoteFetchPromise) {
      remoteFetchPromise = fetch(REMOTE_ENDPOINT, {
        ...
        .then(async (r) => {
          if (!r.ok)
            throw new Error(`Remote fetch failed: ${r.status} ${r.statusText}`);
          ...
          if (
            ...
            return (data as { applications?: RawApplication[] })
              .applications as RawApplication[];
          if (Array.isArray((data as { data?: unknown[] }).data))
            return (data as { data?: RawApplication[] })
              .data as RawApplication[];
          return null;
        })
        .catch((err) => {
          console.warn(
            "[applications] Remote fetch error, falling back to local mock:",
            err.message || err
          );
...
function normalizeStage(type?: string): Application["stage"] {
  ...
  if (t === "financial-aid" || t === "financialaid" || t === "finaid")
...
function getFraudDetails(obj: unknown): RawApplication["fraud_details"] {
  if (obj && typeof obj === "object" && "fraud_details" in obj) {
    ...
    Object.entries(fd as Record<string, unknown>).forEach(([k, v]) => {
      if (Array.isArray(v)) {
        const strings = (v as unknown[]).filter(
...
function normalize(raw: RawApplication[]): {
  queue: Application[];
  ...
  raw.forEach((app, idx) => {
    ...
    const parsedFraudScore =
      app.fraud_score != null ? Number(app.fraud_score) : undefined;
    const fraudScorePct =
      parsedFraudScore != null && !Number.isNaN(parsedFraudScore)
        ? Math.round(parsedFraudScore * 100)
        : undefined;
    ...
    const base: Application = {
      ...
      studentId:
        app.application_id ||
        `ST-${String(app.id || idx + 1).padStart(4, "0")}`,
      name:
    ...
    if (fd) {
      if (Array.isArray(fd.application_behaviour))
        flagsFromDetails.push(...fd.application_behaviour);
      if (Array.isArray(fd.address)) flagsFromDetails.push(...fd.address);
      if (Array.isArray(fd.academic_record))
        flagsFromDetails.push(...fd.academic_record);
    }
...
export async function fetchProcessedApplications(): Promise<Application[]> {
  return apiClient.get(
    ...
    async () => (await transform()).processed
  );
}
...
function isRawApplication(obj: unknown): obj is RawApplication {
  ...
  return (
    "application_id" in rec ||
    "id" in rec ||
    "first_name" in rec ||
    "application_status" in rec
  );
}
...
function buildApplicationFromRaw(
  raw: RawApplication,
  ...
  const parsedFraudScore =
    raw.fraud_score != null ? Number(raw.fraud_score) : undefined;
  const riskScore =
    parsedFraudScore != null && !Number.isNaN(parsedFraudScore)
      ? Math.round(parsedFraudScore * 100)
      : undefined;
  ...
  if (fd)
    Object.values(fd).forEach((arr) => {
      if (Array.isArray(arr)) flags.push(...arr);
    });
  ...
  return {
    id: String(
      (actualRaw as { id?: string | number } | undefined)?.id ??
        (actualRaw as { application_id?: string })?.application_id ??
    ...
    name:
      [
        (actualRaw as { first_name?: string })?.first_name,
        (actualRaw as { last_name?: string })?.last_name,
      ]
        .filter(Boolean)
    ...
    stage,
    timestamp:
      (actualRaw as { updated_at?: string })?.updated_at ||
      (actualRaw as { created_at?: string })?.created_at ||
      new Date().toISOString(),
    status:
      statusRaw === "processed"
        ? "processed"
        : (statusRaw as Application["status"]) || "submitted",
...
function buildApplicationDetailFromRaw(
  raw: RawApplication,
  applicationIdFallback: string
): ApplicationDetail {
  // If raw has both application_id and data, drill down to data
  ...
  return {
    application_id:
      (actualRaw as RawApplication).application_id ||
      String(
        (actualRaw as { id?: string | number }).id || applicationIdFallback
      ),
...
export async function fetchApplicationById(
  ...
  try {
    const res = await fetch(
      `${REMOTE_ENDPOINT}/${encodeURIComponent(applicationId)}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    console.warn(
      "[applications] Remote summary fetch error:",
...
export async function fetchApplicationDetailById(
  applicationId: string
  ...
  try {
    ...
    console.warn(
      "[applications] Remote detail fetch error:",
      e instanceof Error ? e.message : e
    );
  }
  // Fallback to list search (remote)
...
// you get application id and payload just make a put api call
export async function update_application(applicationId: any, payload: any) {
  try {
    const res = await fetch(
      `${REMOTE_ENDPOINT}/${encodeURIComponent(applicationId)}`,
      {
    ...
    throw new Error(
      `Failed to update application: ${res.status} ${res.statusText}`
    );
  } catch (e) {
    console.warn(
      "[applications] Update application error:",
      e instanceof Error ? e.message : e
    );
  }
}
